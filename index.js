class Board {
  constructor(rows, cols, bombs, r, c) {
    ...
    this.bombs = bombs;
    this.displayBoard = Array.from({ length: rows }, () =>
      Array(cols).fill(-1)
    );
  ...
  bombDirections() {
    ...
    for (let i = 0; i < this.rows; i++) {
      for (let j = 0; j < this.cols; j++) {
        if (this.board[i][j] !== -1) {
          // Left side check
          if (i - 1 >= 0) nearbyBombs += this.incrementBomb(i - 1, j);
          // Right check
          if (i + 1 !== this.rows) nearbyBombs += this.incrementBomb(i + 1, j);
          // Up Check
          if (j - 1 >= 0) nearbyBombs += this.incrementBomb(i, j - 1);
          if (j + 1 !== this.cols) nearbyBombs += this.incrementBomb(i, j + 1);
          // Up Left Check
          if (i - 1 >= 0 && j - 1 >= 0)
  ...
  incrementBomb(row, col) {
    if (this.board[row][col] === -1) return 1;
    return 0;
  }

  makeMove(i, j) {
    if (this.gameover !== "N") return;

    if (this.board[i][j] != -1) {
      if (this.board[i][j] === 0) this.expand(i, j);
      else this.displayBoard[i][j] = this.board[i][j];
    } else {
      this.gameover = "D";
  ...
  expand(i, j) {
    ...
    // Left Check
    if (i - 1 >= 0 && this.displayBoard[i - 1][j] === -1) this.expand(i - 1, j);
    // Right check
    if (i + 1 !== this.rows && this.displayBoard[i + 1][j] === -1)
      this.expand(i + 1, j);
    // Up Check
    if (j - 1 >= 0 && this.displayBoard[i][j - 1] === -1) this.expand(i, j - 1);
...
for (let i = 0; i < mode.length; i++) {
  ...
  element.onclick = function () {
    if (element.innerHTML === "Beginner") heavyReset(9, 9, 10);
    if (element.innerHTML === "Intermediate") heavyReset(16, 16, 40);
    if (element.innerHTML === "Expert") heavyReset(16, 35, 99);
  };
}
...
function newGame(rows, cols, originalbombs) {
  ...
  var arrayofButtons = Array.from({ length: buttonrows }, () =>
  ...
  document.documentElement.style.setProperty(
    "--dynamic-width",
    `calc(${buttonsizewidth}px * ${buttoncols})`
  );

  document.documentElement.style.setProperty(
    "--dynamic-height",
    `calc(${buttonsizeheight}px * ${buttonrows})`
  );
  ...
  for (let index = 0; index < buttonrows; index++) {
    for (let j = 0; j < buttoncols; j++) {
      ...
      para.oncontextmenu = function (event) {
        ...
        if (firstPressed) {
          if (gameboard.gameover !== "N") return;
        }
      ...
      para.onmousedown = function (event) {
        ...
        if (firstPressed) if (gameboard.gameover !== "N") return;
    ...
    for (let i = 0; i < arrayofButtons.length; i++) {
      for (let j = 0; j < arrayofButtons[0].length; j++) {
        ...
        if (gameboard.displayBoard[i][j] > 0) {
          ...
          resetButton.style.backgroundImage = "url('./icons/death.png')";
          if (
            arrayofButtons[i][j].style.backgroundImage ===
            'url("./icons/flag.png")'
          )
          ...
          arrayofButtons[i][j].style.backgroundImage =
            "url('./icons/bomb.gif')";
          arrayofButtons[i][j].style.backgroundSize = `${
            buttonsizewidth + 20
          }px ${buttonsizeheight + 20}px`;
          arrayofButtons[i][j].onmousedown = null;
          arrayofButtons[i][j].onmouseup = null;
          arrayofButtons[i][j].onmouseout = null;
        }
